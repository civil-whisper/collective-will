        ...
        if tier == "english_reasoning":
            ...
            if 'Policy discussion: "foreign-economic-sanctions"' in prompt:
                return LLMResponse(
                    text=json.dumps({
                        "ballot_question": (
                            "Further refine whether increased foreign sanctions "
                            "should advance as a public proposition."
                        ),
                        "ballot_question_fa": (
                            "مشخص شود آیا افزایش تحریم‌های خارجی باید به عنوان "
                            "یک پیشنهاد عمومی جلو برود"
                        ),
            ...
            if "Create a refinement draft for this cluster." in prompt:
                return LLMResponse(
                    text=json.dumps({
                        "refinement_draft": (
                            "Require a public proposal that defines the scope, "
                            "duration, and legal conditions for any increase in "
                            "foreign sanctions."
                        ),
                        "refinement_draft_fa": (
                            "یک پیشنهاد عمومی باید دامنه، مدت و شرایط هر "
                            "افزایش تحریم خارجی را مشخص کند"
                        ),
                    ...
                    [
                        {
                            ...
                            "description": (
                                "اعتصاب گسترده و هماهنگ برای کاهش منابع حکومت، "
                                "با ریسک فشار اقتصادی بر مردم."
                            ),
                            "description_en": (
                                "Use broad coordinated strikes to reduce regime "
                                "resources, with the trade-off of economic "
                        ...
                        },
                        {
                            ...
                            "description": (
                                "اعتصاب‌های محدود و هدفمند در بخش‌های کلیدی، "
                                "با اثر کمتر بر زندگی روزمره اما فشار کمتر."
                            ),
                            "description_en": (
                                "Use narrower strikes in key sectors, reducing "
                                "spillover on daily life but also lowering "
                                "overall pressure."
                            ),
                        },
                    ]
...
async def test_replay_submissions_generates_cluster_artifacts() -> None:
    ...
    assert strike_cluster["ballot_question"] == (
        "Adopt coordinated labor strikes to economically pressure the Iranian regime."
    )
