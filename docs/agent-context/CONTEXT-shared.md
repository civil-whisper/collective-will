...
| **Canonicalization model** | `gpt-5.4-mini` (any language → structured English). Runs inline at submission time with batch fallback. Accepts positions, questions, concerns, and expressions of interest about policy topics — not just explicit stances. Fallback: Claude Sonnet 4.6. Canonicalization prompts are split into a stable instruction prefix (`_CANONICALIZATION_INSTRUCTIONS`) plus dynamic context and input, so provider-side prefix caching maximizes reuse. Open-key context is capped by `CANONICALIZATION_CONTEXT_MAX_ENTRIES` and `CANONICALIZATION_CONTEXT_SUMMARY_CHARS`. |
...
| **LLM model strategy** | OpenAI-first for now: all primary completion tiers default to `gpt-5.4-mini`, with Claude Sonnet 4.6 as fallback. This is config-backed and temporary if provider behavior changes. Model selection is config-backed — switch any tier via env without code changes. Web search grounding supported for Anthropic, Google, and OpenAI. Provider-side prompt caching enabled by default (`LLM_PROMPT_CACHING_ENABLED=true`): Anthropic system blocks get `cache_control`, OpenAI automatic cache hits are captured. Cache telemetry exposed in `LLMResponse` and replay stats. |
...
| **Pre-ballot signatures** | Multi-stage approval is required before ballot: concerns first gather signatures/endorsements as agenda-setting items. Only clusters classified as `ballot-ready` may advance from endorsement into final voting. Broad concerns can remain visible as `discussion-only` or `needs-refinement` items while the system autonomously refines them into clearer proposition drafts. Refinement generation should run only for clusters with at least one `needs-refinement` member; pure `discussion-only` clusters stay as civic discussion topics with no option set. Those refinement drafts are stored at the cluster level with confidence + clarification flags and are visible on the website, but they do not automatically promote a cluster into voting. Refinement outputs must stay faithful to the cluster evidence: if a draft invents an actor/jurisdiction, changes direction, or adds an ungrounded target, the pipeline drops the draft and keeps the cluster in clarification-needed state. Ballot-question wording follows the same principle: if endorsement wording invents a new actor or target that is not grounded in the member submissions, the pipeline replaces it with a safer actor-neutral fallback before exposing it publicly. |
...
| **Ops observability console** | Add a separate `/ops` diagnostics surface for runtime health/events. In dev/staging it may appear in top navigation; in production it must be admin-auth gated and feature-flagged. Show structured, redacted operational events (health checks, recent errors, job status, webhook/email transport status), not raw container logs. An unauthenticated `/ops/monitor-health` endpoint provides active Telegram webhook verification, DB/scheduler/email health, recent error/warning counts, and a redacted summary of recent backend exceptions for the external monitoring script. |
| **Ops email monitoring** | A systemd timer runs `scripts/monitor-ops.sh` every 5 minutes on the VPS. It queries `/ops/monitor-health` for each environment, classifies failures, sends targeted alert emails via Resend on first occurrence (dedup by fingerprint for `OPS_ALERT_DEDUP_MINUTES`), and sends one daily heartbeat email at `OPS_HEARTBEAT_HOUR_UTC` when all services are healthy. Recent backend exceptions are promoted into a `backend_runtime` monitored service so voice/API/channel failures (for example `V003` voice service errors) trigger alerts instead of being count-only. Config: `OPS_ALERT_EMAILS`, `OPS_HEARTBEAT_HOUR_UTC`, `OPS_MONITOR_LOOKBACK_MINUTES`, `OPS_ALERT_DEDUP_MINUTES`. Monitor state is persisted in `/var/lib/collective-will-monitor/`. |
| **Infrastructure** | Njalla domain is registered (WHOIS privacy). **Hosting as of 2026-08-19:** Hetzner Cloud CX23 (`5.75.158.200`, Falkenstein, identity-verified) for pre-traction cost. Privacy-first VPS (1984.is / Njalla) remains the target before the pilot carries real Iranian user data at volume. Cloudflare (Free plan) is **active** as the edge proxy — DNS, CDN, DDoS protection, and Bot Fight Mode enabled. Caddy `trusted_proxies static` is configured with all Cloudflare IP ranges + `trusted_proxies_strict` to preserve real client IPs. Production domain serves a static 503 maintenance page until the production stack is deployed (staging is the active environment). Deploy pipeline includes preflight checks, pull retries with backoff, and post-deploy health gates. Origin IP should stay unpublished (Cloudflare proxy + firewall limited to Cloudflare ranges on IPv4 and IPv6). Operator failover playbook + standby VPS must be documented. **Next origin move:** `scripts/provision-vps.sh` (SSH bootstrap + env + first deploy); optional `scripts/repoint-origin-dns.sh` when `CLOUDFLARE_API_TOKEN` is set. Day-to-day deploys stay on GitHub Actions against `VPS_HOST`. |
...
├── deploy/                      # VPS compose, Caddyfile, public env, BUNDLE, remote-entry.sh
├── scripts/
│   ├── bootstrap-server.sh      # One-time host prep (deploy user, Caddy, sudoers)
│   ├── provision-vps.sh         # Next-box orchestrator (bootstrap + env + first deploy)
│   ├── sync-deploy-bundle.sh    # Copy deploy/BUNDLE to the VPS (used by Actions too)
│   ├── push-env.sh              # Merge public env + secrets onto the VPS
│   └── repoint-origin-dns.sh    # Optional Cloudflare API: grey/orange A records
├── docker-compose.yml
├── pyproject.toml
├── .env.example
├── .gitignore
