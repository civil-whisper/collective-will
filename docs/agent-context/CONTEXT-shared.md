...
| **Canonicalization model** | Gemini 3.1 Pro (any language → structured English). Runs inline at submission time with batch fallback. Accepts positions, questions, concerns, and expressions of interest about policy topics — not just explicit stances. Fallback: Claude Sonnet. |
...
| **LLM model strategy** | Gemini-first: all primary tiers default to `gemini-3.1-pro-preview` for best performance-to-cost ratio. All fallbacks default to `claude-sonnet-4-20250514`. Model selection is config-backed — switch any tier via env without code changes. |
